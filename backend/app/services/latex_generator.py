    ...
    def _generate_title_section(self, analysis: DocumentAnalysis) -> str:
        ...
        if analysis.authors:
            ...
            if self.template == LatexTemplate.ACM:
                ...
                parts.append("\\institute{Institution Name \\email{author@domain.com}}")
...
{email}}}"""
  ...
  \\country{{{country}}}
            ...
            # Springer format with \\fnm and \\sur
            author_parts = []
            for i, name in enumerate(authors.names):
                ...
                if i in authors.corresponding_indices:
                    author_parts.append(f"\\author*{{\\fnm{{{first_name}}} \\sur{{{last_name}}}}}")
                else:
                    author_parts.append(f"\\author{{\\fnm{{{first_name}}} \\sur{{{last_name}}}}}")
            ...
            if authors.affiliations:
                affiliation = authors.affiliations[0]
                author_parts.append(f"""\\affil{{\\orgdiv{{Department}}, \\orgname{{{affiliation}}}, 
